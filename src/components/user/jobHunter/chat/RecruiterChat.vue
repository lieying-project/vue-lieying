        ...
        <div class="chat-container">
            <div class="chat-wrap">
                ...
                <ul class="chat-user-list">
                    <li class="chat-user" ref="chatUser" v-for="(jobHunter,index) in $store.state.chatJobHunters" :key="index"
...
<!--                                      v-if="recruiter.company!=null">{{recruiter.company.name}}</span>&nbsp;&nbsp;|&nbsp;&nbsp;-->
            ...
            <div class="chat">
                <div class="title">
                    <p class="fl" v-if="currentJobHunter!=null&&currentJobHunter.id!=0">
                        <span class="name">{{currentJobHunter.name}}</span>
                ...
                <ul class="chat-message-list" v-if="currentJobHunter.id!=0">
...
export default {
  ...
  methods: {
    setCurrentRecruiterIndex(index) {
      this.currentJobHunter = this.$store.state.chatJobHunters[index]
    },
    changeUserBackground(index, i) {  //修改用户背景
      if (index == this.currentJobHunter.id) { //如果当前用户被选中,则不修改
        return
      }
    ...
    sendMessage(){
      this.chat={
        jobHunter:{
          id:this.currentJobHunter.id,
          photo: this.currentJobHunter.photo
        },
        recruiter:{
          id:this.recruiterId,
          photo: this.currentJobHunter.photo
    ...
    websocketonmessage(e){//数据接收
      ...
      if (Array.isArray( this.chatData)){
        this.$store.state.recruiterChats= this.chatData
      }else{
        this.$store.state.recruiterChats.push(this.chatData)
      }
    },
  ...
  created() {
    ...
    this.initWebSocket("ws://localhost:8080/lieying/websocket/"+this.recruiterId+"/recruiter")
    this.$store.dispatch("getChatJobHuntersByRecruiterIdAction",this.recruiterId)
  }
}
</script>
